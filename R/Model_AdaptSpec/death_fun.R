...
death_fun = function(xi_prop,xi_cur,tmin,Smax,Beta,tau, timeseries, sigmasalpha, D, B, nu_0){
  ...
  # Determine q_beta
  par_c1 = betapar(tsq = tau_c1, sigmasalpha = sigmasalpha, D = D, ts_seg = timeseries[(xi_cur[m_star-1]+1):(xi_cur[m_star])], B = B)
  par_c2 = betapar(tsq = tau_c2, sigmasalpha = sigmasalpha, D = D, ts_seg = timeseries[(xi_cur[m_star]+1):(xi_cur[m_star+1])], B = B)
  
  # get q_betas
  q_beta1 = mvtnorm::dmvnorm(beta_c1, mean = par_c1$mean, sigma = par_c1$cov)
  q_beta2 = mvtnorm::dmvnorm(beta_c2, mean = par_c2$mean, sigma = par_c2$cov)
  
  # q_betap
  par_p = betapar(tsq = tau_p, sigmasalpha = sigmasalpha, D = D, ts_seg = timeseries[(xi_cur[m_star-1]+1):(xi_cur[m_star+1])], B = B)
  beta_p = c(mvtnorm::rmvnorm(1, mean = par_p$mean, sigma = par_p$cov))
  q_betap = mvtnorm::dmvnorm(beta_p, mean = par_p$mean, sigma = par_p$cov)
  ...
  if(U < A){
    # Accept
    if(length(xi_cur) == 3){
      Beta_prop = matrix(beta_p, nrow = length(beta_p), ncol = 1)
    }else if(m_star == 1){
      Beta_prop = cbind(beta_p, Beta[,(m_star+2):ncol(Beta)])
    } else if(m_star == (ncol(Beta)-1)){
      Beta_prop = cbind(Beta[,1:(m_star-1)], beta_p)
    }else{
      Beta_prop = cbind(Beta[,1:(m_star-1)], beta_p, Beta[,(m_star+2):ncol(Beta)])
    }
