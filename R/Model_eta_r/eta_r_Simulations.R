# Multiple Single Stationary Time Series
library(mvtnorm)
library(progress)
library(fda)
library(bayesplot)
...
source("R/Model_eta_r/sim_eta_r.R")
...
# post_pred = array(data = NA, dim = c(length(test_omega), iter - burnin, R, Sim))
post_pred = array(data = NA, dim = c(length(test_omega), iter, R, Sim))

# Create vector object to store simultaneous coverage checks for spectral density
Simult_truespec = array(NA,dim = c(R,Sim))
...
for(i in 1:Sim){
  ...
  funccall = sim_eta_r(n = n, iter = iter, phi = phi, R = R, B = B)
  # extract
  Theta = funccall$Theta
  #extract_beta 
  bb_beta_array = funccall$bb_beta_array
  # dim(Theta)
  av_perio = funccall$av_perio
  
  # Loop over the replicates to create test_specdens
  # store in array each iteration
  test_specdens = array(data = NA, c(length(test_omega), iter, R))
  for(r in 1:R){
    test_specdens[,,r] = exp(test_Psi %*% t(bb_beta_array[,,r]))
  }
  # test_specdens = exp(test_Psi %*% t(Theta[ ,-(B+2)]))
  #plot(x = test_omega, y = test_specdens[,1000,1])
  ...
  if(TRUE){ 
    for(r in 1:R){
      ...
      # hist(post_pred[1,])
      post_pred[,,r,i] = matrix(rgamma(nrow(test_specdens[,,r]) * ncol(test_specdens[,,r]), R, R / c(test_specdens[,,r])),
                                nrow(test_specdens[,,r]),
                                ncol(test_specdens[,,r]))
    }
  ...
  if(TRUE){
    ...
    #store Simult_specdens
    for(r in 1:R){
      Simult_specdens = fbplot(fit = log(test_specdens[,,r]), x = test_omega, xlim = range(test_omega), ylim = range(log(test_specdens[,,r])), prob = c(0.95, 0.5, .05),
                               xlab = "omega",
                               factor = 1.5,
                               barcol = "dodgerblue" ,
                               ...
                               fullout = TRUE,
                               plot = FALSE)
      deepcurve = log(test_specdens[,,r])[, Simult_specdens$depth >= quantile(Simult_specdens$depth, .05)]
      uppercurve = apply(deepcurve, 1, FUN = max)
      lowercurve = apply(deepcurve, 1, FUN = min)
      Simult_truespec[r,i] = as.numeric(all((log(true_spec) <= uppercurve) & (log(true_spec) >= lowercurve)))
...
Simult_specdens = fbplot(fit = log(post_pred[,,1,Sim]), x = test_omega, xlim = range(test_omega), ylim = range(log(post_pred[,,1,Sim])), prob = c(0.95, 0.5, .05),
                         xlab = "omega",
                         factor = 1.5,
                         barcol = "dodgerblue" ,
                         col = c("purple", "orange", "violetred3"),
